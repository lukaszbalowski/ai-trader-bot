...
def render_dashboard(configs, state, active_markets, trades, live_data, balance, init_balance, logs, errors, trade_history):
    ...
    def market_sort_key(cfg):
        ...
        sym_order = {'BTC': 0, 'ETH': 1, 'SOL': 2, 'XRP': 3}
    ...
    for cfg in sorted_configs:
        ...
        # --- NOWY MECHANIZM STATUSÓW ---
        market_status = state.get('market_status', {}).get(tk, "[loading...]")
        status_color = YLW if "paused" in market_status or "mid join" in market_status else GRN
        status_fmt = f"{status_color}{market_status}{RST}"
        ...
        if tk in active_markets:
            ...
            out.append(f"{BLD}[{cfg.get('ui_key', '-')}] {cfg['symbol']} {cfg['timeframe']}{RST} {status_fmt} {ver_txt} | {meta_str} | Strike: ${target:,.2f}{session_str}")
            out.append(f"   ├─ Live: ${adj_p:,.2f} | D: {diff_trend} ${abs(diff):.2f}{floating_str}")
            
            if market_trades:
                out.append(f"   ├─ L2 -> UP: {b_up*100:04.1f}¢ | DOWN: {b_dn*100:04.1f}¢")
            ...
            out.append(f"{BLD}[{cfg.get('ui_key', '-')}] {cfg['symbol']} {cfg['timeframe']}{RST} {status_fmt} | Waiting for new market... (Live: ${live_p:,.2f}){session_str}")

    # --- PORTFOLIO STATUS (Sekcja przywrócona) ---
    ...
    out.append(f"\n{BLD}[Controls] {RST}'q' -> Emergency exit | 'd' -> Tech Dump | 'mrX' -> Resume market X | 'msX' -> Pause market X")
    sys.stdout.write("\n".join(out) + "\n")
    sys.stdout.flush()
